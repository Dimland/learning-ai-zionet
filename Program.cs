...
do
{
    ...
    if (debugMode)
    {
        logger.LogDebug($"User input: {userInput}");
    }
    ...
    if (userInput.Contains("repository path") && userInput.Contains("set"))
    {
        ...
        if (pathMatch.Success)
        {
            ...
            if (debugMode)
            {
                logger.LogDebug($"Detected repository path: {currentRepositoryPath}");
            }
        }
    }
    ...
    try
    {
        ...
        await foreach (var chunk in streamingResponse)
        {
            ...
            if (chunk.FunctionResults != null && chunk.FunctionResults.Count > 0 && debugMode)
            {
                foreach (var functionResult in chunk.FunctionResults)
                {
                    logger.LogDebug($"Function called: {functionResult.FunctionName}");
                    logger.LogDebug($"Function result: {functionResult.Content}");
                }
            }
        }
        ...
        if (debugMode && !string.IsNullOrEmpty(currentRepositoryPath))
        {
            try
            {
                    ...
                    Directory.Exists(Path.Combine(currentRepositoryPath, ".git")))
                {
                    ...
                    logger.LogDebug($"Repository info - Current branch: {repo.Head.FriendlyName}");
                    logger.LogDebug($"Repository info - Last commit: {repo.Head.Tip.MessageShort}");
                }
            }
            catch (Exception ex)
            {
                logger.LogDebug($"Error accessing repository: {ex.Message}");
            }
        }
    }
    catch (Exception ex)
    {
        ...
        if (debugMode)
        {
            logger.LogError(ex, "Error in processing request");
        }
